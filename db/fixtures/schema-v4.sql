...
-- Церковнослужители, которых уже вводили. Отдельная таблица, а не выборка
-- из person_mention: заказчик 27.08.2026 просил не набирать причт, а выбирать
-- из списка — «сделать кнопку выбора из выпадающего списка нужного церковника,
-- а если в списке его нет, то после ввода его руками он добавляется в базу
-- и появляется в списке».
--
-- Причт в одном приходе меняется редко: за год-два это те же три человека.
-- Поэтому список короткий и его можно показать целиком, не дожидаясь ввода
-- первых букв, — в отличие от подсказок по персонам, где счёт идёт на тысячи.
CREATE TABLE clergy_index (
  id            INTEGER PRIMARY KEY,
  iof           TEXT    NOT NULL,
  iof_norm      TEXT    NOT NULL,
  rank          TEXT,
  uses          INTEGER NOT NULL DEFAULT 0,
  last_used_at  TEXT,
  UNIQUE (iof, rank)
);
CREATE INDEX ix_clergy_uses ON clergy_index (uses DESC, iof);
