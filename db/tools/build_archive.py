...
ВАЖНО: архив отправляется Роману ОДИН раз. Программа отказывает в повторной
загрузке того же файла, но другой архив (пересобранный позже из того же
Excel) сольётся снова и сложит счётчики частоты второй раз — подсказки из
Excel перевесят набранное в программе. Второй архив — только после решения,
как сливать идемпотентно (см. HANDOFF).
